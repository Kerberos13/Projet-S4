...
At the moment, only .jpg, .bmp and .wav files are supported. In the cas of a .wav file, the spectrogram will automatically
...
error message should appear and invite you to run the setup.py script. Please note that you should run this setup script with 
root privileges.
...
and margin values, lines' width and color, and choose between the CNN network or the SDA network with the toggle button.
The compute button will then launch the calculation and display the spectrogram.
...
module that will reassemble the complete spectrogram. If the GUI is used, then the ProjetS4 module will display the GUI and
proceed similarly to the main module. Almost all modules have some functions in common that are in the tools module.
